...
public class AddCourses implements Serializable {
    ...
    private List<SimpleGenericDto<Long, String, String>> lstCourses;

    @Autowired
    private IAddCourseService iAddCourseService;

    public List<SimpleGenericDto<Long, String, String>> loadLstCourses(){
        ...
        lstCourses = new ArrayList<SimpleGenericDto<Long, String, String>>();
        lstCourses.addAll(outputGetLstCoursesDto.getLstCourses().stream().sorted(Comparator.comparing(SimpleGenericDto::getKey)).collect(Collectors.toList()));
        return lstCourses;
    }
